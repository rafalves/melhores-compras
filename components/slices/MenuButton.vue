  ...
  <ClientOnly>
    <div class="ml-6 md:hidden">
      ...
      <button v-if="isOpen" @click="isOpen = false"
        class="fixed inset-0 bg-gray-500 opacity-75 z-40 cursor-default"></button>
      <div v-if="isOpen" class="fixed top-0 left-0 w-2/5 h-screen bg-blue-500 z-50">
        <nav class="m-4">
          <NuxtLink class="block" v-for="category in props.categories" :key="category.id"
