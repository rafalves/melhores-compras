    ...
    <div
      ...
      <div class="order-1 flex flex-col h-[170px] tablet:h-fit">
        ...
        <div class="flex justify-center overflow-hidden hover:opacity-80 relative flex-1">
          ...
          <span class="absolute bottom-0 right-0  mr-2 mb-2 px-1 rounded text-slate-700 text-sm bg-yellow-400 z-10">{{
            props.data.attributes.seller
          }}</span>
          ...
          <div class="flex justify-start gap-3 items-baseline">
            <span class="text-orange-600 font-semibold text-xl my-0">{{
              (props.data.attributes.price).toLocaleString('pt-br', {
                style: 'currency',
                currency: 'BRL'
              }) }}</span> <span v-if="props.data.attributes.discount"
              class=" bg-black text-white px-2 rounded h-[21px]">{{
      ...
      <div class="order-4 w-full">
        ...
        <div class="flex justify-between items-baseline pt-1 mx-2 my-0 ">
          <div class="text-xs">
            <Icon name="eos-icons:hourglass" color="#4D5765" class="-mt-2 mr-1" size="20px" />
            <span>{{ useFormatData(props.data.attributes.updatedAt) }}</span>
          </div>
          <div class="flex">
