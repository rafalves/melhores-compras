<template>
  <div class="mb-5">
    <div v-if="!enable" class="relative max-w-350 max-h-250 flex">
      <span class="absolute  text-white p-2 font-semibold drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]"> {{
        embedParsed.title }}</span>
    ...
    <iframe class="w-full aspect-video h-full max-h-[500px]" v-else="!enable" width="500" height="300" :src="src"
      allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
      allowfullscreen data-rocket-lazyload="fitvidscompatible" data-ll-status="loaded" />
  </div>
</template>
